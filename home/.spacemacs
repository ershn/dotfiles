...
you should place your code here."
  ...
  (spacemacs/set-leader-keys "o f" 'select-frame-by-name)

  (spacemacs/set-leader-keys "h m" 'man)
  (evil-define-key 'motion Man-mode-map
    "C-k" (lambda () (interactive) (Man-previous-section 1) (recenter 0))
    "C-j" (lambda () (interactive) (Man-next-section 1) (recenter 0))
  ...
  (spacemacs/set-leader-keys "p s" 'projectile-switch-project)
  ...
  (defun find-in-parent-dirs (file-name &optional dir)
    (when-let (dir (or dir (file-name-directory buffer-file-name)))
      (catch 'exit
        (while t
          (if (file-exists-p (concat dir file-name))
              (throw 'exit dir)
            (let ((parent-dir (file-name-directory (directory-file-name dir))))
              (when (equal dir parent-dir)
                (throw 'exit nil))
              (setq dir parent-dir)))))))
  )
...
(custom-set-variables
 ...
 '(safe-local-variable-values
   (quote
    ((eval set
           (make-local-variable
            (quote prettier-js-command))
           (expand-file-name "node_modules/.bin/prettier"
                             (find-in-parent-dirs "node_modules")))
